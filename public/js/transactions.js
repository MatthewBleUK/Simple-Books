        ...
        $.ajax({
            ...
            success:function(response) {
                if(response) {
                    ...
                    var minusSign = document.getElementById("minus-sign").innerText;
                    ...
                    cell1.innerHTML = '<input type="checkbox" name="transaction">';
                    cell2.innerHTML = response.date;
                    cell3.innerHTML = response.name;
                    cell4.innerHTML = '<span>'+minus+'</span><span>£</span><span><span class="transaction-amount">'+response.amount+'</span>';
                    cell5.innerHTML = '<span class="category">'+response.category+'</span>';
                    cell6.innerHTML = response.tags;
                    cell7.innerHTML = '<span onclick="openEditTransaction('+response.id+')"><i class="arrow right"></i></span>';
    ...
    $("input:checkbox[name='transaction']:checked").each(function () {        
        ...
        $.ajax({
            ...
            success: function () {
                ...
                let row = $("input:checkbox[name='transaction']:checked").closest('tr').attr('id');
                ...
                incomeDashboardVal = parseFloat(incomeDashboardVal) - parseFloat(amountColumn);
                $('#income').text(incomeDashboardVal.toFixed(2));

                // minus from total dashboard
                totalDashboardVal = parseFloat(totalDashboardVal) - parseFloat(amountColumn);
                }

                // if category is Expense
                if(categoryColumn.indexOf("Expense") !== -1) {
                    // minus from expense dashboard
                    expenseDashboardVal = parseFloat(expenseDashboardVal) - parseFloat(amountColumn);
            ...
            complete: function (data) {
                ...
                if(rowCount == 1) {
                   $('<span id="no-transactions">There are no transactions</span>').insertAfter('#transactions-table');
                }
            }
        });        
    });
    ...
    if(errors.length <= 0) {   
        ...
        $.ajax( {
            ...
            success:function(response) {   
                ...
                let oldAmount = parseFloat($('#'+id.val()+' .transaction-amount').text());
                ...
                if(category.val() == 'Expense') {
                    $('#'+id.val()+' td:eq(3)').html('<span>-</span><span>£</span><span class="transaction-amount">'+amount+'</span>');
                } else if (category.val() == 'Income') {
                    $('#'+id.val()+' td:eq(3)').html('<span>£</span><span class="transaction-amount">'+amount+'</span>');
                }
